...
NSString *const kWXSSKeyChainErrorDomain = @"com.samsoffes.WXSSKeyChain";

NSString *const kWXSSKeyChainAccountKey = @"acct";
NSString *const kWXSSKeyChainCreatedAtKey = @"cdat";
NSString *const kWXSSKeyChainClassKey = @"labl";
NSString *const kWXSSKeyChainDescriptionKey = @"desc";
NSString *const kWXSSKeyChainLabelKey = @"labl";
NSString *const kWXSSKeyChainLastModifiedKey = @"mdat";
NSString *const kWXSSKeyChainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef WXSSKeyChainAccessibilityType = NULL;
#endif

@interface WXSSKeyChain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation WXSSKeyChain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = WXSSKeyChainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kWXSSKeyChainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (WXSSKeyChainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return WXSSKeyChainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (WXSSKeyChainAccessibilityType) {
		CFRelease(WXSSKeyChainAccessibilityType);
	}
	WXSSKeyChainAccessibilityType = accessibilityType;
}
#endif
